...
static void IN_LoadUnloadPics(void (*callback)(const char *lumpname,
  ...
  switch (gameepisode) {
  case 1: callback(DEH_String("MAPE1"), 0, &patchINTERPIC); break;
  case 2: callback(DEH_String("MAPE2"), 0, &patchINTERPIC); break;
  case 3: callback(DEH_String("MAPE3"), 0, &patchINTERPIC); break;
  default: break;
  }
...
void IN_Drawer(void) {
  ...
  case 0: // draw stats
    IN_DrawStatBack();
    switch (gametype) {
    case SINGLE: IN_DrawSingleStats(); break;
    case COOPERATIVE: IN_DrawCoopStats(); break;
    case DEATHMATCH: IN_DrawDMStats(); break;
    }
    break;
  ...
  case 3: // waiting before going to the next level
    ...
    break;
  default: I_Error("IN_lude:  Intermission state out of range.\n"); break;
  }
}
    ...
    for (i = 0; i < MAXPLAYERS; i++) {
      if (playeringame[i]) {
        V_DrawShadowedPatch(40,
                            ((ypos << FRACBITS) + dSlideY[i] * intertime) >>
                                FRACBITS,
                            W_CacheLumpNum(patchFaceOkayBase + i, PU_CACHE));
