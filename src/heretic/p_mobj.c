...
void P_SpawnMapThing(mapthing_t *mthing) {
  ...
  {
    P_AddBossSpot(mthing->x << FRACBITS,
                  mthing->y << FRACBITS,
...
void P_SpawnPuff(fixed_t x, fixed_t y, fixed_t z) {
  ...
  switch (PuffType) {
  case MT_BEAKPUFF:
  case MT_STAFFPUFF: puff->momz = FRACUNIT; break;
  case MT_GAUNTLETPUFF1:
  case MT_GAUNTLETPUFF2: puff->momz = (fixed_t)(.8 * FRACUNIT);
  default: break;
  }
}
...
void A_ContMobjSound(mobj_t *actor) {
  switch (actor->type) {
  case MT_KNIGHTAXE: S_StartSound(actor, sfx_kgtatk); break;
  case MT_MUMMYFX1: S_StartSound(actor, sfx_mumhed); break;
  default: break;
  }
}
