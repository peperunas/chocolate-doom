...
static void NET_CL_ParsePacket(net_packet_t *packet) {
  ...
  if (NET_Conn_Packet(&client_connection, packet, &packet_type)) {
    ...
    switch (packet_type) {
    case NET_PACKET_TYPE_SYN: NET_CL_ParseSYN(packet); break;

    case NET_PACKET_TYPE_REJECTED: NET_CL_ParseReject(packet); break;

    case NET_PACKET_TYPE_WAITING_DATA: NET_CL_ParseWaitingData(packet); break;

    case NET_PACKET_TYPE_LAUNCH: NET_CL_ParseLaunch(packet); break;

    case NET_PACKET_TYPE_GAMESTART: NET_CL_ParseGameStart(packet); break;

    case NET_PACKET_TYPE_GAMEDATA: NET_CL_ParseGameData(packet); break;
