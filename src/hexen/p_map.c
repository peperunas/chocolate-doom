...
void P_LineAttack(mobj_t *t1,
  ...
  if (P_PathTraverse(t1->x,
                     ...
                     PTR_ShootTraverse)) {
    switch (PuffType) {
    case MT_PUNCHPUFF: S_StartSound(t1, SFX_FIGHTER_PUNCH_MISS); break;
    case MT_HAMMERPUFF:
    case MT_AXEPUFF:
    case MT_AXEPUFF_GLOW: S_StartSound(t1, SFX_FIGHTER_HAMMER_MISS); break;
    case MT_FLAMEPUFF:
  ...
  if (!in->d.line->special) {
    ...
    if (openrange <= 0) {
      if (usething->player) {
        ...
        case PCLASS_FIGHTER: sound = SFX_PLAYER_FIGHTER_FAILED_USE; break;
        case PCLASS_CLERIC: sound = SFX_PLAYER_CLERIC_FAILED_USE; break;
        case PCLASS_MAGE: sound = SFX_PLAYER_MAGE_FAILED_USE; break;
        case PCLASS_PIG: sound = SFX_PIG_ACTIVE1; break;
...
boolean PTR_PuzzleItemTraverse(intercept_t *in) {
  ...
  if (in->isaline) { // Check line
    if (in->d.line->special != USE_PUZZLE_ITEM_SPECIAL) {
      ...
      if (openrange <= 0) {
        ...
        if (PuzzleItemUser->player) {
          ...
          case PCLASS_FIGHTER: sound = SFX_PUZZLE_FAIL_FIGHTER; break;
          case PCLASS_CLERIC: sound = SFX_PUZZLE_FAIL_CLERIC; break;
          case PCLASS_MAGE: sound = SFX_PUZZLE_FAIL_MAGE; break;
          default: sound = SFX_NONE; break;
          }
        }
    ...
    if (P_PointOnLineSide(PuzzleItemUser->x,
                          PuzzleItemUser->y,
