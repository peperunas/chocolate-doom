...
static Menu_t SkillMenu =
    {120, 44, DrawSkillMenu, 5, SkillItems, 2, MENU_CLASS};
...
static Menu_t OptionsMenu =
    {88, 30, DrawOptionsMenu, 5, OptionsItems, 0, MENU_MAIN};
...
static const char *GammaText[] = {TXT_GAMMA_LEVEL_OFF,
                                  TXT_GAMMA_LEVEL_1,
                                  TXT_GAMMA_LEVEL_2,
                                  TXT_GAMMA_LEVEL_3,
                                  TXT_GAMMA_LEVEL_4};
  ...
  if (MenuActive == false) {
    if (askforquit) {
      MN_DrTextA(QuitEndMsg[typeofask - 1],
                 160 - MN_TextAWidth(QuitEndMsg[typeofask - 1]) / 2,
                 80);
      if (typeofask == 3) {
      ...
      if (typeofask == 4) {
        MN_DrTextA(SlotText[quickload - 1],
                   160 - MN_TextAWidth(SlotText[quickload - 1]) / 2,
                   90);
        MN_DrTextA("?", 160 + MN_TextAWidth(SlotText[quicksave - 1]) / 2, 90);
      }
...
boolean MN_Responder(event_t *event) {
  ...
  if (!FileMenuKeySteal) {
    ...
    if (key == KEY_ESCAPE) {
      M_StringCopy(SlotText[currentSlot],
                   oldSlotText,
