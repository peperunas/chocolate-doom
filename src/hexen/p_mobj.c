...
void P_ExplodeMissile(mobj_t *mo) {
  ...
  case MT_SORCBALL1:
  case MT_SORCBALL2:
  case MT_SORCBALL3: S_StartSound(NULL, SFX_SORCERER_BIGBALLEXPLODE); break;
  case MT_SORCFX1: S_StartSound(NULL, SFX_SORCERER_HEADSCREAM); break;
  default:
...
void P_FloorBounceMissile(mobj_t *mo) {
  if (P_HitFloor(mo) >= FLOOR_LIQUID) {
    ...
    case MT_SORCBALL1:
    case MT_SORCBALL2:
    case MT_SORCBALL3: break;
    default: P_RemoveMobj(mo); return;
    }
  }
  switch (mo->type) {
  case MT_SORCFX1:
  ...
  case MT_SGSHARD0:
    ...
    break;
  default: mo->momz = FixedMul(mo->momz, -0.7 * FRACUNIT); break;
  }
  mo->momx = 2 * mo->momx / 3;
  mo->momy = 2 * mo->momy / 3;
  if (mo->info->seesound) {
    ...
    case MT_SORCBALL3:
      ...
      break;
    default: S_StartSound(mo, mo->info->seesound); break;
    }
    S_StartSound(mo, mo->info->seesound);
  }
  //      P_SetMobjState(mo, mobjinfo[mo->type].deathstate);
        ...
        if (mo->momz < -GRAVITY * 8 && !(mo->flags2 & MF2_FLY)) { // squat down
          ...
          if (mo->momz < -23 * FRACUNIT) {
            ...
            case PCLASS_CLERIC:
              ...
              break;
            case PCLASS_MAGE: S_StartSound(mo, SFX_PLAYER_MAGE_GRUNT); break;
            default: break;
            }
...
static void PlayerLandedOnThing(mobj_t *mo, mobj_t *onmobj) {
  ...
  if (mo->momz < -23 * FRACUNIT) {
    ...
    S_StartSound(mo, SFX_PLAYER_LAND);
    switch (mo->player->class) {
    case PCLASS_FIGHTER: S_StartSound(mo, SFX_PLAYER_FIGHTER_GRUNT); break;
    case PCLASS_CLERIC: S_StartSound(mo, SFX_PLAYER_CLERIC_GRUNT); break;
    case PCLASS_MAGE: S_StartSound(mo, SFX_PLAYER_MAGE_GRUNT); break;
    default: break;
    }
...
void P_SpawnPlayer(mapthing_t *mthing) {
  ...
  switch (p->class) {
  case PCLASS_FIGHTER: mobj = P_SpawnMobj(x, y, z, MT_PLAYER_FIGHTER); break;
  case PCLASS_CLERIC: mobj = P_SpawnMobj(x, y, z, MT_PLAYER_CLERIC); break;
  case PCLASS_MAGE: mobj = P_SpawnMobj(x, y, z, MT_PLAYER_MAGE); break;
  default: I_Error("P_SpawnPlayer: Unknown class type"); return;
  }
...
void P_SpawnMapThing(mapthing_t *mthing) {
  ...
  case MT_ZLYNCHED_NOHEART: P_SpawnMobj(x, y, ONFLOORZ, MT_BLOODPOOL); break;
  default: break;
  }
...
void P_SpawnPuff(fixed_t x, fixed_t y, fixed_t z) {
  ...
  switch (PuffType) {
  case MT_PUNCHPUFF: puff->momz = FRACUNIT; break;
  case MT_HAMMERPUFF: puff->momz = .8 * FRACUNIT; break;
  default: break;
  }
  PuffSpawned = puff;
}
...
int P_HitFloor(mobj_t *thing) {
  ...
  case MT_SPLASH:
  case MT_SLUDGECHUNK: return (FLOOR_SOLID);
  default: break;
  }
...
mobj_t *P_SpawnMissile(mobj_t *source, mobj_t *dest, mobjtype_t type) {
  ...
  case MT_MNTRFX2: // Minotaur floor fire missile
    ...
    break;
  case MT_CENTAUR_FX: z = source->z + 45 * FRACUNIT; break;
  case MT_ICEGUY_FX: z = source->z + 40 * FRACUNIT; break;
  case MT_HOLY_MISSILE: z = source->z + 40 * FRACUNIT; break;
  default: z = source->z + 32 * FRACUNIT; break;
  }
  z -= source->floorclip;
...
mobj_t *P_SpawnMissileXYZ(fixed_t    x,
                          fixed_t    y,
                          fixed_t    z,
                          mobj_t *   source,
                          mobj_t *   dest,
...
mobj_t *P_SpawnMissileAngle(mobj_t *   source,
                            mobjtype_t type,
                            angle_t    angle,
                            fixed_t    momz) {
  fixed_t z;
  ...
  case MT_ICEGUY_FX2: // Secondary Projectiles of the Ice Guy
    ...
    break;
  case MT_MSTAFF_FX2: z = source->z + 40 * FRACUNIT; break;
  default: z = source->z + 32 * FRACUNIT; break;
  }
  z -= source->floorclip;
...
mobj_t *P_SpawnMissileAngleSpeed(mobj_t *   source,
                                 mobjtype_t type,
                                 angle_t    angle,
                                 fixed_t    momz,
                                 fixed_t    speed) {
...
mobj_t *P_SPMAngleXYZ(mobj_t *   source,
                      fixed_t    x,
                      fixed_t    y,
                      fixed_t    z,
                      mobjtype_t type,
...
mobj_t *P_SpawnKoraxMissile(fixed_t    x,
                            fixed_t    y,
                            fixed_t    z,
                            mobj_t *   source,
                            mobj_t *   dest,
