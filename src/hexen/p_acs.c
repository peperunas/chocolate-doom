...
static int ReadOffset(void) {
  ...
  ACSAssert(offset < ActionCodeSize,
            "invalid lump offset: %d >= %d",
            offset,
            ActionCodeSize);
  return offset;
}
...
void P_LoadACScripts(int lump) {
  ...
  M_snprintf(EvalContext,
             sizeof(EvalContext),
             "header parsing of lump #%d",
             lump);
...
void T_InterpretACS(acs_t *script) {
  ...
  do {
    M_snprintf(EvalContext,
               sizeof(EvalContext),
               "script %d @0x%x",
               ACSInfo[script->infoIndex].number,
               PCodeOffset);
    cmd = ReadCodeInt();
    M_snprintf(EvalContext,
               sizeof(EvalContext),
