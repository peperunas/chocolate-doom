...
static int NextCalibrateStage(txt_joystick_axis_t *joystick_axis) {
  switch (joystick_axis->config_stage) {
  case CONFIG_CENTER: return CONFIG_STAGE1;
  ...
  case CONFIG_STAGE2: return CONFIG_CENTER;
  }
...
static int EventCallback(SDL_Event *event, TXT_UNCAST_ARG(joystick_axis)) {
  ...
  if (event->jbutton.which == SDL_JoystickInstanceID(joystick_axis->joystick) &&
    ...
    switch (joystick_axis->config_stage) {
    default:
    case CONFIG_STAGE1: advance = CalibrateAxis(joystick_axis); break;

    case CONFIG_STAGE2: advance = SetButtonAxisPositive(joystick_axis); break;
    }
...
void TXT_ConfigureJoystickAxis(txt_joystick_axis_t *        joystick_axis,
  ...
  TXT_SetWindowAction(joystick_axis->config_window,
                      TXT_HORIZ_CENTER,
  ...
  // is closed.
  TXT_SignalConnect(joystick_axis->config_window,
                    "closed",
                    CalibrateWindowClosed,
                    joystick_axis);
...
static void
TXT_JoystickAxisMousePress(TXT_UNCAST_ARG(widget), int x, int y, int b) {
