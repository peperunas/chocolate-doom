...
static boolean WritePipe(net_packet_t *packet) {
  DWORD bytes_written;
  BOOL  ok = WriteFile(midi_process_in_writer,
                      packet->data,
                      packet->len,
                      &bytes_written,
                      NULL);
...
static boolean ExpectPipe(net_packet_t *packet) {
  ...
  do {
    ...
    ok = PeekNamedPipe(midi_process_out_reader,
                       NULL,
                       0,
                       NULL,
                       &pipe_buffer_read,
    ...
    ok = ReadFile(midi_process_out_reader,
                  pipe_buffer,
                  packet->len,
                  &pipe_buffer_read,
                  NULL);
...
boolean I_MidiPipe_InitServer() {
  ...
  if (!CreatePipe(&midi_process_in_reader,
                  &midi_process_in_writer,
                  &sec_attrs,
                  0)) {
  ...
  if (!CreatePipe(&midi_process_out_reader,
  ...
  // the executable name.
  M_snprintf(params_buf,
             sizeof(params_buf),
             "%d %Iu %Iu",
             snd_samplerate,
             (size_t)midi_process_in_reader,
             (size_t)midi_process_out_writer);
  cmdline =
