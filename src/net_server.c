...
static void NET_SV_SendWaitingData(net_client_t *client) {
  ...
  memcpy(&wait_data.wad_sha1sum,
         &controller->wad_sha1sum,
         sizeof(sha1_digest_t));
  memcpy(&wait_data.deh_sha1sum,
         &controller->deh_sha1sum,
         sizeof(sha1_digest_t));
...
NET_SV_ParseSYN(net_packet_t *packet, net_client_t *client, net_addr_t *addr) {
  ...
  default: NET_Log("server: error: wrong magic number: %d", magic); return;
  }
...
static void
NET_SV_SendTics(net_client_t *client, unsigned int start, unsigned int end) {
  net_packet_t *packet;
  unsigned int  i;
...
static void NET_SV_Packet(net_packet_t *packet, net_addr_t *addr) {
  ...
  NET_Log("server: packet from %s; type %d",
  ...
  if (packet_type == NET_PACKET_TYPE_SYN) {
    ...
    case NET_PACKET_TYPE_GAMESTART:
      ...
      break;
    case NET_PACKET_TYPE_LAUNCH: NET_SV_ParseLaunch(packet, client); break;
    case NET_PACKET_TYPE_GAMEDATA: NET_SV_ParseGameData(packet, client); break;
...
static void NET_SV_PumpSendQueue(net_client_t *client) {
  ...
  NET_Log("server: send tics %d-%d to %s",
          starttic,
          endtic,
          NET_AddrToString(client->addr));
...
void NET_SV_Run(void) {
  ...
  switch (server_state) {
  case SERVER_WAITING_LAUNCH: break;

  case SERVER_WAITING_START: CheckStartGame(); break;
