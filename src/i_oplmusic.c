...
static void SetVoiceInstrument(opl_voice_t *    voice,
  ...
  LoadOperatorData(voice->op2 | voice->array,
                   &data->carrier,
                   true,
                   &voice->car_volume);
  LoadOperatorData(voice->op1 | voice->array,
                   &data->modulator,
                   !modulating,
                   &voice->mod_volume);
...
static void SetChannelVolume(opl_channel_data_t *channel,
                             unsigned int        volume,
                             boolean             clip_start);
...
static void ControllerEvent(opl_track_data_t *track, midi_event_t *event) {
  ...
  */
  ...
  case MIDI_CONTROLLER_ALL_NOTES_OFF: AllNotesOff(channel, param); break;
...
static void MetaEvent(opl_track_data_t *track, midi_event_t *event) {
  ...
  case MIDI_META_SEQUENCER_SPECIFIC: break;
  ...
  case MIDI_META_END_OF_TRACK: break;

  default:
#ifdef OPL_MIDI_DEBUG
    fprintf(stderr,
            "Unknown MIDI meta event type: %u\n",
...
static void ProcessEvent(opl_track_data_t *track, midi_event_t *event) {
  ...
  case MIDI_EVENT_CONTROLLER: ControllerEvent(track, event); break;

  case MIDI_EVENT_PROGRAM_CHANGE: ProgramChangeEvent(track, event); break;

  case MIDI_EVENT_PITCH_BEND: PitchBendEvent(track, event); break;
  ...
  case MIDI_EVENT_SYSEX_SPLIT: break;
