...
boolean P_GiveWeapon(player_t *player, weapontype_t weapon, boolean dropped) {
  ...
  if (player->weaponowned[weapon])
    ...
    switch (weapon) {
    case wp_elecbow: player->weaponowned[wp_poisonbow] = true; break;

    case wp_hegrenade: player->weaponowned[wp_wpgrenade] = true; break;

    case wp_mauler: player->weaponowned[wp_torpedo] = true; break;
...
void P_TouchSpecialThing(mobj_t *special, mobj_t *toucher) {
  ...
  case MT_SPECTRE_E:
  case MT_ENTITY:
  case MT_SUBENTITY: P_DamageMobj(toucher, NULL, NULL, 5); return;
  default: break;
  }
  ...
  // 1 Gold
  case SPR_COIN: P_GiveInventoryItem(player, SPR_COIN, MT_MONY_1); break;
...
void P_KillMobj(mobj_t *source, mobj_t *target) {
  ...
  */
  ...
  if (target->player) {
    ...
    if (netgame) {
      ...
      while (1) {
        ...
        if (item == MT_MONY_1) {
          loot = P_SpawnMobj(target->x,
                             target->y,
                             target->z + (24 * FRACUNIT),
                             MT_MONY_25);
  ...
  if (!item) {
    ...
    switch (target->type) {
    case MT_ORACLE: item = MT_MEAT; break;

    case MT_PROGRAMMER: item = MT_SIGIL_A; break;

    case MT_PRIEST: item = MT_JUNK; break;

    case MT_BISHOP: item = MT_AMINIBOX; break;

    case MT_PGUARD:
    case MT_CRUSADER: item = MT_ACELL; break;

    case MT_RLEADER: item = MT_AAMMOBOX; break;

    case MT_GUARD1:
    case MT_REBEL1:
    case MT_SHADOWGUARD: item = MT_ACLIP; break;

    case MT_SPECTRE_B: item = MT_SIGIL_B; break;

    case MT_SPECTRE_C: item = MT_SIGIL_C; break;

    case MT_SPECTRE_D: item = MT_SIGIL_D; break;

    case MT_SPECTRE_E: item = MT_SIGIL_E; break;
