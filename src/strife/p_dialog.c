...
void P_DialogDoChoice(int choice) {
  ...
  if (choice != dialogmenu.numitems - 1 && candochoice) {
    ...
    if (item < 0 || P_GiveItemToPlayer(dialogplayer,
...
void P_DialogStart(player_t *player) {
  ...
  if (!linetarget) {
    P_AimLineAttack(player->mo,
                    player->mo->angle + (ANG90 / 16),
                    (128 * FRACUNIT));
    if (!linetarget)
      P_AimLineAttack(player->mo,
                      player->mo->angle - (ANG90 / 16),
  ...
  // get bye text
  switch (rnd) {
  case 2: byetext = DEH_String("BYE!"); break;
  case 1: byetext = DEH_String("Thanks, Bye!"); break;
  default:
  case 0: byetext = DEH_String("See you later!"); break;
  }

  DEH_snprintf(dialoglastmsgbuffer,
               sizeof(dialoglastmsgbuffer),
               "%d) %s",
               i + 1,
               byetext);
}
