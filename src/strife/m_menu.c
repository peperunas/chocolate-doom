...
const char *cursorName[8] = {"M_CURS1",
                             "M_CURS2",
                             "M_CURS3",
                             "M_CURS4",
                             "M_CURS5",
...
menu_t ReadDef3 =
    {read3_end, &ReadDef2, ReadMenu3, M_DrawReadThis3, 250, 185, 0};
...
menuitem_t LoadMenu[] = {{1, "", M_LoadSelect, '1'},
                         {1, "", M_LoadSelect, '2'},
                         {1, "", M_LoadSelect, '3'},
                         {1, "", M_LoadSelect, '4'},
                         {1, "", M_LoadSelect, '5'},
...
void M_ReadSaveStrings(void) {
  ...
  for (i = 0; i < load_end; i++) {
    ...
    if (handle == NULL) {
      M_StringCopy(savegamestrings[i],
...
void M_DrawSound(void) {
  ...
  M_DrawThermo(SoundDef.x,
               SoundDef.y + LINEHEIGHT * (voice_vol + 1),
               16,
               voiceVolume);

  M_DrawThermo(SoundDef.x,
               SoundDef.y + LINEHEIGHT * (sfx_mouse + 1),
               16,
               mouseSensitivity);
}
...
boolean M_Responder(event_t *ev) {
  ...
  if (saveStringEnter) {
    ...
    case KEY_ESCAPE:
      saveStringEnter = 0;
      I_StopTextInput();
      M_StringCopy(savegamestrings[quickSaveSlot],
                   saveOldString,
