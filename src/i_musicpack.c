...
static void ParseOggFile(file_metadata_t *metadata, FILE *fs) {
  ...
  for (offset = 0; offset < 100 * 1024; ++offset) {
    ...
    if (!memcmp(buf + 1, "vorbis", 6)) {
      switch (buf[0]) {
      case OGG_ID_HEADER: ParseOggIdHeader(metadata, fs); break;
      case OGG_COMMENT_HEADER: ParseVorbisComments(metadata, fs); break;
      default: break;
      }
    }
  }
...
static boolean I_MP_InitMusic(void) {
  ...
  } else if (Mix_OpenAudioDevice(snd_samplerate,
                                 AUDIO_S16SYS,
                                 2,
                                 1024,
                                 NULL,
...
static void I_MP_PlaySong(void *handle, boolean looping) {
  ...
  if (Mix_PlayMusic(current_track_music, loops) == -1) {
    fprintf(stderr,
            "I_MP_PlaySong: Error starting track: %s\n",
            Mix_GetError());
  }
}
