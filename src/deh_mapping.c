...
boolean DEH_SetMapping(deh_context_t *context,
  ...
  switch (entry->size) {
  case 1: *((uint8_t *)location) = value; break;
  case 2: *((uint16_t *)location) = value; break;
  case 4: *((uint32_t *)location) = value; break;
  default:
...
void DEH_StructSHA1Sum(sha1_context_t *context,
                       deh_mapping_t * mapping,
                       void *          structptr) {
  int i;
  ...
  for (i = 0; mapping->entries[i].name != NULL; ++i) {
    ...
    switch (entry->size) {
    case 1: SHA1_UpdateInt32(context, *((uint8_t *)location)); break;
    case 2: SHA1_UpdateInt32(context, *((uint16_t *)location)); break;
